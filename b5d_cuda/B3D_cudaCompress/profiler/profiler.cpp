...
#ifdef _WIN32
    //#define NOMINMAX // Aaron edit, as per https://exceptionshub.com/stdmax-expected-an-identifier.html
    #include <windows.h>
#endif
#ifdef __linux
    #include <sys/time.h>
...
void Profiler::endFrame( ) {
    ...
    uint64 curTime = getTime( );
    m_maxFrameTimeSinceLastOutput = max( m_maxFrameTimeSinceLastOutput, curTime - m_frameStartTime ); //std::max
    m_totalMaxFrameTime = max( m_totalMaxFrameTime, m_maxFrameTimeSinceLastOutput ); // std::max
