...
// perform scalar DWT on one channel of a ushort array with srcChannelCount interleaved channels (ie elemStride is srcChannelCount)
// dpSource points to the correct channel within the first data element
// srcChannelCount has to be between 1 and 4
CUCOMP_DLL void dwtFloat2DForwardFromUshort(
	float* dpDest, float* dpBuffer, const ushort* dpSource,
	int sizeX, int sizeY, int srcChannelCount = 1,
	int dstRowPitch = 0,
	int srcRowPitch = 0,
	cudaStream_t stream = 0);
CUCOMP_DLL void dwtFloat3DForwardFromUshort(
	float* dpDest, float* dpBuffer2, float* dpBuffer1, const ushort* dpSource,
	int sizeX, int sizeY, int sizeZ, int srcChannelCount = 1,
	int dstRowPitch = 0, int dstSlicePitch = 0,
	int srcRowPitch = 0, int srcSlicePitch = 0,
	cudaStream_t stream = 0);
// perform scalar IDWT into one channel of a ushort array with dstChannelCount interleaved channels (ie elemStride is dstChannelCount)
// dpDest points to the correct channel within the first data element
// dstChannelCount has to be between 1 and 4
CUCOMP_DLL void dwtFloat2DInverseToUshort(
	ushort* dpDest, float* dpBuffer, const float* dpSource,
	int sizeX, int sizeY, int dstChannelCount = 1,
	int dstRowPitch = 0,
	int srcRowPitch = 0,
	cudaStream_t stream = 0);
CUCOMP_DLL void dwtFloat3DInverseToUshort(
